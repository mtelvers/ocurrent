...
module Job : sig
  ...
  (** [wait_for_log_data t] suspends the fiber until the next time log data
...
module Process : sig
  ...
  val exec :
    ...
    ?env:string array ->
    cancellable:bool ->
    job:Job.t -> string list ->
    unit or_error
  (** [exec ~job cmd] runs [cmd] via {!Eio.Process}, with output to [job]'s log.
