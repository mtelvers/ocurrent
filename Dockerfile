...
RUN cd ~/opam-repository && git fetch -q origin master && git reset --hard 835aa544ea8a6a628f2b02609336ae9de5472277 && opam update
ADD --chown=opam *.opam /src/
WORKDIR /src
RUN opam pin add -yn prometheus.1.3 "https://github.com/mtelvers/prometheus.git#eio" && \
    opam pin add -yn prometheus-app.1.3 "https://github.com/mtelvers/prometheus.git#eio" && \
    opam pin add -yn current_web.dev "./" && \
    ...
    opam pin add -yn current_term.dev "./" && \
