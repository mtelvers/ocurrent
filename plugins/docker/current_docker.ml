...
module Make () (E : sig
  ...
  let docker_context = E.docker_context
  ...
  let pull_cache = PullC.create ~caps:E.caps
  let peek_cache = PeekC.create ~caps:E.caps
  let build_cache = BC.create ~caps:E.caps
  let run_cache = RC.create ~caps:E.caps
  let pread_cache = PrC.create ~caps:E.caps
  let tag_cache = TC.create ~caps:E.caps
  let push_cache = Push_cache.create ~caps:E.caps
  let service_cache = SC.create ~caps:E.caps
  let compose_cache = CC.create ~caps:E.caps
  let compose_cli_cache = CCC.create ~caps:E.caps
  let push_manifest_cache = MC.create ~caps:E.caps
  ...
  module Raw = struct
    ...
    let build ?level ?schedule ?timeout ?(squash=false) ?(buildx = false) ?dockerfile ?path ?pool ?(build_args=[]) ~pull commit =
      ...
      in
      BC.get build_cache ?schedule { Build.pull; pool; timeout; level; git = E.git }
