...
module Raw = struct
  ...
  let compose_v2 ?docker_compose_file ?path ?(pull=true) ?(detach=true) ?(up_args = []) ~docker_context ~project_name ~images commit =
    let docker_compose_file =
      ...
      | Some (`Contents c) -> `Contents c
    in
    CV2.set Compose_v2.{ pull } { Compose_v2.Key.commit; docker_compose_file; path; docker_context; detach; up_args; project_name } { Compose_v2.Value.images }
...
module Make (Host : S.HOST) = struct
  ...
  let compose_v2 ?docker_compose_file ?path ?pull ?detach ?up_args ~project_name ~images src =
    ...
    Raw.compose_v2 ?docker_compose_file ?path ?pull ?detach ?up_args ~docker_context ~project_name ~images commit
end
